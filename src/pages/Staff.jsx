...
import { Plus, Trash2, X, UserCog, KeyRound } from 'lucide-react';
...
const Staff = () => {
  ...
  const load = () => authService.getStaffList().then(setStaff);
      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-border bg-card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-foreground">New Staff Account</h2>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-3">
            <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" className={inputClass} required />
      ...
      {showPasswordForm && (
        <div className="mb-6 rounded-xl border border-border bg-card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={() => setShowPasswordForm(false)} className="text-muted-foreground hover:text-foreground">
              <X size={18} />
            </button>
          </div>
          {pwdError && (
            <div className="mb-4 rounded-lg bg-destructive/10 px-4 py-3 text-sm text-destructive">{pwdError}</div>
          )}
          {pwdSuccess && (
            <div className="mb-4 rounded-lg bg-primary/10 px-4 py-3 text-sm text-primary">{pwdSuccess}</div>
          )}
          <form
            onSubmit={async (e) => {
              e.preventDefault();
          ...
          >
            <input type="password" value={currentPwd} onChange={(e) => setCurrentPwd(e.target.value)} placeholder="Current password" className={inputClass} required />
            <input type="password" value={newPwd} onChange={(e) => setNewPwd(e.target.value)} placeholder="New password" className={inputClass} required />
            <input type="password" value={confirmPwd} onChange={(e) => setConfirmPwd(e.target.value)} placeholder="Confirm new password" className={inputClass} required />
            ...
            {staff.length === 0 ? (
              ...
              staff.map((s) => (
                <tr key={s._id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground capitalize">{s.role}</td>
